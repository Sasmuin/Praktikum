...
\subsection{$^{137}$Cs Szintillator}
Danach wird das Spektrum der $^{137}$Cs-Probe betrachtet, das in \cref{cs_sz} zu sehen ist. In diesem Fall gibt es nur einen Übergang und damit auch nur einen Full Energy Peak. Allerdings ist auch hier wieder der Mittelpunkt dieses Peaks mit $E_{FEP1} = \SI{663,5 \pm 0,6}{keV}$ etwa 2keV größer als der Literaturwert. Daraus lässt sich schließen, dass es mit großer Wahrscheinlichkeit zu einem Fehler bei der Kalibrierung, insbesondere $E_{0}$ gekommen ist, da konstant alle Werte einige keV größer als die Literaturwerte sind. Die Compton- und Backscatterkante passen allerdings wieder gut ins Spektrum rein.
