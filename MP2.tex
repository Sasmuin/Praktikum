...
Ziel bei dieser Untersuchung ist es die Gitterkonstante a des fcc-Gitters zu bestimmen und ihre lineare Abhängigkeit von der Temperatur zu überprüfen. Außerdem soll geschaut werden, ob die Kernaussagen die Debye mathematisch hergeleitet hat auch experimentell bestätigt werden können. Weiterhin wird der Ordnungsparameter während der Phasenumwandlung bei etwa 375°C bestimmt. Der Phasenübergang findet von hoher Temperatur bzw. hoher Unordnung zu kleiner Temperatur bzw. größere Ordnung statt. Dabei wird die Dynamik beim Phasenübergang bestimmt und geschaut mit wecher Potenz sich die Ordnung im Verlauf der Zeit ausbildet. Die Potenz sollte nach theoretischen Vorhersagen $t^4$ entsprechen, jedoch wurde ein Ergebnis gefunden, welches den theoretsichen Vorhersagen nicht entsprach, sodass hier verschiedene Probleme diskutiert wurden, die diese Differenz erklären.
...
Außerdem lassen sich die Gitterkonstanten mithilfe von \cref{bragg} und \cref{d} berechnen. Beispielhaft wird dies hier in \cref{tab} für 400°C gemacht:
\begin{table}[h]
