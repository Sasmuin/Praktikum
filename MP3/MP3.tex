...
Anhand der Messwerte lässt sich einiges über das Verhalten des Stoffes herausfinden. Nur die Verformungen bei einem Grad von $e^1-e^3$ sind etwa 2,7\% - 20,1\% konnten beschrieben werden.\cref{A3} konnte einen exponentiellen Zuwachs von Körnern bei steigendem Verformungsgrad beobachten. Doch
    \centering
...
\subsection{Diskussion}
Anhand der Messwerte lässt sich einiges über das Verhalten des Materials herausfinden. \cref{A3} kann in drei Bereiche gegliedert werden. Den ersten Bereich, welcher mathematisch nicht beschrieben werden konnte. In diesem Bereich konnte wurde einAbfall an der Größe der Körner beobachtet. Im zweiten Bereich konnte ein strikter mathematischer Zusammenhang nachgewiesen werden und den dritten Bereich, wo wieder eine mathematische Modellierung fehlte. Hier konnte jedoch eine Stagnation des Körnerwachstums beobachtet werden.  Nur der zweite Bereich bei einer Verformungen von $e^1-e^3$, was in prozentzahlen etwa 2,7\% - 20,1\% entspricht, konnten beschrieben werden.Genau bei diesem Grad an Verformung erkennt man in \cref{A3} einen exponentiellen Zuwachs von Körnern bei steigendem Verformungsgrad. Als nächstes wird der dritte Fall beschrieben.
Diese Beobachtung ist bei großen Körnerzahlen geschehen. Es scheint, als ob es eine Mindestgröße von Körnern gäbe. Wenn diese Mindestgröße erreicht ist kann das Material beliebig stark verformt werden ohne dass weitere Körnerbildung ansetzt. Das liegt daran, dass das Material nur ein gewisses Maß an Kapazität von Verzerrungsenergie bereit stellt. Anderenfalls zerspringt das Material.
Der erste Fall ist der, dass bei der Referenzprobe (keine Verformung) mehr Körner vorhanden sind, als nach den ersten beiden Verformungsstufen, dass heißt, dass die Körner erst vergrößert werden, bevor weitere durch den Prozess der Rekristallisation entstehen. Bei diesen Werten beginnt der Prozess der Rekristallisation erst bei einer Verzerrungsstärke von 2,7\%. Dies liegt im theoretischen Bereich, ab dem Rekristallisation auftreten könnte. Die Korngröße nimmt zu. 
Woran liegt es, dass die beiden Fälle die nicht mathematisch modelliert wurden auftreten. 
